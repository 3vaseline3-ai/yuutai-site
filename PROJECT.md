...
│   ├── month.html
│   └── stock.html
└── html/                        # 生成されたHTML（+ 静的アセット）
    ├── index.html
    ├── months/{01-12}.html
    ├── stocks/{code}.html
    └── assets/
        ├── yuutai_ext.css       # 月別ページ拡張UI
        └── yuutai_ext.js        # 月別ページ拡張UI
```

## yuutai_ext（月別ページ拡張）

- 月別ページで `O(δ)`（限界利回り）や「早期取得シミュレーション」等を表示するためのJS/CSS
- 早期取得シミュレーションの候補表は「取得金額」を万/円の2段表示（ぱっと見＋詳細）にしている
...
- `/.github/workflows/update-and-deploy.yml` が GitHub Pages にデプロイ
- `main` への push: HTML生成 → デプロイ（データ更新/commit はしない）
- schedule/手動: データ更新 → HTML生成 → commit/push → デプロイ
...
- 2026/01/27: yuutai_ext 早期取得シミュレーションの取得金額表示を改善（万/円の2段表示 + タグ表示）
